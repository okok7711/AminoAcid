...
json_serialize = lambda obj, * \
    args, **kwargs: json.dumps(obj).decode() if _ORJSON else json.dumps(obj)
...
class Bot(ApiClient):
    ...
    def __init__(self, prefix: str = "/", help_command: Optional[UserCommand] = None, **kwargs) -> None:
        self.prefix = prefix
    ...
    def run(self, email: Optional[str] = "", password: Optional[str] = "", *, session: str = "",  **kwargs):
    ...
    async def main_loop(self, email: Optional[str], password: Optional[str], *, session: Optional[str] = ""):
    ...
    async def handle_command(self, message: Message):
        ...
        if args[0] in self.__command_map__:
            await self.__command_map__[args.pop(0)](Context(
                client=self,
                message=message
            ), *args)
        else:
...
class HttpClient(ClientSession):
    ...
    async def request(self, method: str, url: str, *args, **kwargs) -> ClientResponse:
        ...
        headers = kwargs.pop("headers", {}) or get_headers(data=json_serialize(
            kwargs.get("json", {})).encode(), device=self.device, key=self.key, v=self.v)
        if self.session:
            headers["NDCAUTH"] = self.session
        response = await super().request(method, url=(self.base + url if not "wss" in url else url), headers=headers, *args, **kwargs)
