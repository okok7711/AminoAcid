...
class SocketClient:
    ...
    async def reconnect(self):
        ...
        """
        if "socket" not in dir(self): return await self.run()
    ...
    async def run(self):
        ...
        sig = get_headers(data=sign.encode(), key=self.http.key, v=self.http.v)["NDC-MSG-SIG"]
    ...
    async def handle_message(self, message: Message):
        ...
        if message.type == MessageTypes.TEXT:
            if not message.startswith(self.client.prefix): await self.client.events["on_message"](message)
            else: await self.client.handle_command(message)
        else: print(message)
